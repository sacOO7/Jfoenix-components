...
//				newVal.setFont(Font.font("Roboto", FontWeight.BOLD, 24));
		...
		for(int i = 0; i <= 200; i ++) yearsListView.getItems().add(""+(1900+i));
		...
		addEventHandler(KeyEvent.ANY, event -> {
			...
			case HOME:
				...
				break;
			case PAGE_UP:
				if (!backMonthButton.isDisabled()) forward(-1, MONTHS, true, true);
				event.consume();
				break;
			case PAGE_DOWN:
				if (!forwardMonthButton.isDisabled())forward(1, MONTHS, true, true);
				event.consume();
				break;
			case ESCAPE:
				datePicker.hide();
		...
		showTransition = new CachedTransition(yearsListView, new Timeline(
			...
			protected void starting() {super.starting();yearsListView.setVisible(true);};
			{
	...
	protected void updateMonthYearPane() {
		...
		if(datePicker.getValue()!=null) selectedDateLabel.setText(DateTimeFormatter.ofPattern("EEE, MMM yy").format(datePicker.getValue()));
	...
	private void goToDate(LocalDate date, boolean focusDayCell) {
		if (isValidDate(datePicker.getChronology(), date)) {
			...
			if (focusDayCell) findDayCellOfDate(date).requestFocus();			
		}
	}
		...
		for (int row = 0; row < 6; row++) {
			for (int col = 0; col < daysPerWeek; col++) {
				...
				dayCell.addEventHandler(MouseEvent.MOUSE_CLICKED, click->{
					...
					if (click.getButton() != MouseButton.PRIMARY) return;
